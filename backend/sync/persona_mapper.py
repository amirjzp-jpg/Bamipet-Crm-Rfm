...
def guess_persona(row, species: str | None = None) -> PersonaGuess:
    """`row` is one row from the scored RFM+E table (rfm_engine.score_rfm).
    `species` is optional — pass it when Navatel's contact or order schema
    exposes a cat/dog field; leave None to fall back to behavior-only
    inference."""
